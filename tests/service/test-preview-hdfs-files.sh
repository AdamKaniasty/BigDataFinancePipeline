...
if [ -z "$LATEST_FILE" ] || [ "$LATEST_FILE" == "" ]; then
    ...
    echo ""
    echo "Usage:"
    echo "  ./tests/service/test-preview-hdfs-files.sh [HDFS_PATH]"
    echo ""
    echo "Examples:"
    echo "  ./tests/service/test-preview-hdfs-files.sh /data/finance/raw/crypto-trades"
    echo "  ./tests/service/test-preview-hdfs-files.sh /data/finance/raw/crypto-trades/date=2025-12-05/hour=13"
    echo "  ./tests/service/test-preview-hdfs-files.sh /data/finance/raw/crypto (legacy path)"
    exit 0
fi

...
cleanup_spark_job() {
    docker exec "${SPARK_CONTAINER}" bash -lc "pkill -f '[a]nalytics_fx_crypto\\.py' || true" >/dev/null 2>&1 || true
}
...
echo "Running analytics: analytics_fx_crypto.py ${DEBUG_FLAG}"
docker exec "${SPARK_CONTAINER}" timeout --kill-after 30s "${SPARK_JOB_TIMEOUT}" "${BASE_CMD[@]}" "/opt/spark-apps/analytics_fx_crypto.py" ${DEBUG_FLAG}
